...
class KeypleService(
    ...
    private var _state: MutableStateFlow<KeypleServiceState> =
        MutableStateFlow(KeypleServiceState())
    ...
    fun start() {
        loadServerConfig()
        remoteService = KeypleRemoteService(
    ...
    private suspend fun pingServer(): String {
        return try {
            httpClient.get("${serverConfig?.baseUrl()}/card/sam-status") {
    ...
    suspend fun selectCardAndReadContracts(): KeypleResult<CardContracts> {
        return withContext(Dispatchers.IO) {
            ...
            remoteService?.let { service ->
                val result: KeypleResult<String> = executeService(
                    service,
    ...
    private suspend fun <T> executeService(
        ...
        when (val result = remote.executeRemoteService(
            service,
            inputData,
            inputSerializer,
